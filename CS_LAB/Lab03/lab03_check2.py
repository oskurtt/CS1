def runningtime (t1, t2, t3, t4, t5):
    ls = [t1, t2, t3, t4, t5]
    minls = min(ls)
    maxls = max(ls)
    avgls = ((t1 + t2 + t3 + t4 + t5) - int(minls) - int(maxls))/3
    return minls, maxls, avgls
    
a, b, c = runningtime (34, 34, 35, 31, 29)
print ("Stan's stats-- min: {0:1d}, max: {1:1d}, avg: {2:.1f}".format (a, b, c))

a, b, c = runningtime (30, 31, 29, 29, 28)
print ("Kyle's stats-- min: {0:1d}, max: {1:1d}, avg: {2:.1f}".format (a, b, c))

a, b, c = runningtime (36, 31, 32, 33, 33)
print ("Cartman's stats-- min: {0:1d}, max: {1:1d}, avg: {2:.1f}".format (a, b, c))

a, b, c = runningtime (33, 32, 34, 31, 35)
print ("Kenny's stats-- min: {0:1d}, max: {1:1d}, avg: {2:.1f}".format (a, b, c))

a, b, c = runningtime (27, 29, 29, 28, 30)
print ("Bebe's stats-- min: {0:1d}, max: {1:1d}, avg: {2:.1f}".format (a, b, c))
