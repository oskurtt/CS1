...
class MyApp(object):
    ...
    def draw(self):
        ...
        def helper(this_x, this_y, this_length):
            if this_length < 5:
                return 
            self.draw_lines(this_x, this_y, this_length*.5)
            helper(this_x, this_y + (this_length/2), this_length/2)
            helper(this_x, this_y - (this_length/2), this_length/2)
            helper(this_x + (this_length/2), this_y, this_length/2)
            helper(this_x - (this_length/2), this_y, this_length/2)
            # call the draw line function with the param
            
            # calculate the 4 new centers using the param
            
            # call helper 4 times using the newly calculatd var
            
        helper(self.width/2, self.height/2,min(self.width, self.height)/4)
